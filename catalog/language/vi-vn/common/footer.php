<?php
// Text
$_['text_information']  = 'Thông tin';
$_['text_service']      = 'Chăm sóc khách hàng';
$_['text_extra']        = 'Chức năng khác';
$_['text_contact']      = 'Liên hệ';
$_['text_return']       = 'Trả hàng';
$_['text_sitemap']      = 'Sơ đồ trang';
$_['text_manufacturer'] = 'Thương hiệu';
$_['text_voucher']      = 'Phiếu quà tặng';
$_['text_affiliate']    = 'Đại lý';
$_['text_special']      = 'Khuyến mãi';
$_['text_account']      = 'Tài khoản của tôi';
$_['text_order']        = 'Lịch sử đơn hàng';
$_['text_wishlist']     = 'Danh sách yêu thích';
$_['text_newsletter']   = 'Thư thông báo';
$_['text_powered']      = ' %s &copy; %s';
$_['text_return_policy']     = 'Chính sách đổi trả';
$_['text_about_us'] = 'Giới thiệu';
$_['text_privacy_policy'] = 'Chính sách bảo mật';
$_['text_terms_of_service'] = 'Điều khoản dịch vụ';
$_['text_about_footer'] = 'CANDY LOVE SHOP chuyên cung cấp dòng sản phẩm phòng the cao cấp đến bạn. Những sản phẩm shop cung cấp gồm:  Gel Bôi Trơn, Bao Cao Su và Đồ Chơi Người Lớn. Tất cả các sản phẩm đều đã được Shop kiểm tra chất lượng và rất an toàn cho người sử dụng.';
$_['text_link'] = 'Liên kết';